...
class TestFreeFloatingVehicleSharingInterruptionWhileWaitingAnswerFromRH(unittest.TestCase):
    def setUp(self):
        """Initiates the test.
        """
    ...
    def create_supervisor(self, sc):
        ...
        demand = BaseDemandManager([User("U0", [0, 2000], [1000, 1000], Time("07:00:00")),
            ...
            User("U2", [0, 1500], [0, 2000], Time("07:01:00"), pickup_dt=Dt(minutes=6))])
    ...
    def test_interruption_while_waiting_answer_from_RH_1(self):
        """Test that when a traveler is interrupted while being waiting answer from RH, the
        service's buffer is correctly updated. Case when no actual modification
        of service's buffer is realized.
        """
        ## Create supervisor
        ...
        self.assertGreaterEqual(Time(df1.iloc[-1]['TIME']), Time('07:11:34'))
        self.assertLessEqual(Time(df1.iloc[-1]['TIME']), Time('07:11:34').add_time(flow_dt))
    ...
    def test_interruption_while_waiting_answer_from_RH_2(self):
        """Test that when a traveler is interrupted while being waiting answer from RH, the
        service's buffer is correctly updated. Case when drop node is modified.
        """
        ## Create supervisor
    ...
    def test_interruption_while_waiting_answer_from_RH_3(self):
        """Test that when a traveler is interrupted while being waiting answer from RH, the
        service's buffer is correctly updated. Case when user cancels her request.
        """
        ## Create supervisor
    ...
    def test_interruption_while_waiting_answer_no_alternative(self):
        """Test that when a traveler is interrupted while being waiting answer from RH
        and cannot find any alternative path, she turns DEADEND and properly cancels her
        request.
        """
        ## Create supervisor
