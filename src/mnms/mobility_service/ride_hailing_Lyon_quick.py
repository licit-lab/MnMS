...
from mnms.mobility_service.abstract import AbstractOnDemandMobilityService, AbstractOnDemandDepotMobilityService, \
    Request, compute_path_travel_time, compute_path_nodes_travel_time
...
class RideHailingServiceLyon_quick(AbstractOnDemandMobilityService):
    instances = []
    ...
    def __init__(self,
        ...
        """
        super(RideHailingServiceLyon_quick, self).__init__(id, veh_capacity=1, dt_matching=dt_matching,
    ...
    def add_request(self, user: "User", drop_node: str, request_time: Time) -> None:
        ...
        """
        super(RideHailingServiceLyon_quick, self).add_request(user, drop_node, request_time)
    ...
    def launch_matching_batch(self, dt):
        ...
        for i in range(len(paths)):
            ...
            if veh.activity is not None and veh.activity.activity_type is not ActivityType.STOP:
                ...
                service_dt += Dt(
                    seconds=compute_path_travel_time(veh.activity.path[veh_curr_node_ind_in_path + 1:], self.gnodes,
                                                     self.id))
    ...
    def request_nearest_idle_vehicle_in_radius_fifo(self, user: User, drop_node: str) -> Dt | tuple[
        ...
        """
        """if (user.id == 'U25006') or (user.id == 'U302230'):
        ...
        for i, (veh_path_idle, cost_idle) in enumerate(paths):
            ...
            if total_profit >= threshold:
                ...
                #print("idle time, dist: ", Dt(seconds=cost_idle), len_path_idle)
                #print("service time, dist: ", Dt(seconds=cost_service), len_path_service)
                continue
    ...
    def __dump__(self):
        return {"TYPE": ".".join([RideHailingServiceLyon_quick.__module__, RideHailingServiceLyon_quick.__name__]),
