...
class Supervisor(object):
    ...
    def run_optimization(self, tstart: Time, tend: Time, flow_dt: Dt, affectation_factor: int, optim_factor, update_graph_threshold: float = 0.,
            seed: int = None):
        """Launch a full simulation.
        ...
        while self.tcurrent < tend:
            ...
            for u in new_users:
                if u.path is None:
                    ...
                    self._decision_model.manage_forced_initial_path(u)
            self._decision_model.add_users_for_planning(new_users_for_planning,
                                                        [Event.DEPARTURE] * len(new_users_for_planning))
            ...
            for _ in range(affectation_factor):
                ...
                # Call user flow step
                users_reach_dt_answer = self.call_user_flow_step(flow_dt, users_step)
                self._decision_model.add_users_for_planning(users_reach_dt_answer,
                                                            [Event.MATCH_FAILURE] * len(users_reach_dt_answer))
            ...
            ## Optimization
            if affectation_step % optim_factor==0:

                ## Get data
                for resid, res in self._flow_motor.reservoirs.items():
                    print()
                    print(resid)
                    for mode in res.modes:
                        print(mode, res.dict_speeds[mode],res.dict_accumulations[mode])
                ## Optimize...
